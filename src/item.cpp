	...
	if (it.isRune()) {
		...
		if (it.weaponType == WEAPON_DISTANCE && it.ammoType != AMMO_NONE) {
			...
			if (it.abilities) {
				...
				if (it.abilities->statsPercent[STAT_MAGICPOINTS]) {
					if (begin) {
						begin = false;
						s << " (";
					}
					...
					s << "magic level " << std::showpos << it.abilities->statsPercent[STAT_MAGICPOINTS] << std::noshowpos << '%';
				}
				...
				if (it.abilities->healthGain) {
					...
					if (it.abilities->healthTicks){
						s << "hp reg " << std::showpos << it.abilities->healthGain << std::noshowpos << " p/[" << ((it.abilities->healthTicks) / 1000) << "]s";
					}
					else{
						s << "hp reg " << std::showpos << it.abilities->healthGain << std::noshowpos << " p/[1]s";
					}
				}
				...
				if (it.abilities->manaGain) {
					...
					if (it.abilities->manaTicks){
						s << "mana reg " << std::showpos << it.abilities->manaGain << std::noshowpos << " p/[" << ((it.abilities->manaTicks) / 1000) << "]s";
					}
					else{
						s << "mana reg " << std::showpos << it.abilities->manaGain << std::noshowpos << " p/[1]s";
					}
				}
