...
class Monster final : public Creature
{
	...
	public:
		...
		std::string m_name, m_nameDescription;
		uint64_t m_experience = 0;
		int32_t m_level = 0;
		int32_t m_plusPowerPercent = 0;
		uint64_t m_loot = 1;
		...
		int32_t getMonsterPlusPower() const {
			return m_plusPowerPercent;
		}
		Skulls_t getMonsterSkull() const {
			return m_skull;
		}
		...
		uint32_t getMonsterPlusLoot() const {
			return m_loot;
		}
	...
	private:
		...
		uint64_t getLostExperience() const final {
			return skillLoss ? m_experience : 0;
		}
