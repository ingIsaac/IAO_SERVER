	...
	Creature()
{
	...
	m_level = mType->level;
	int32_t rnd = uniform_random(0, 800);
	int32_t upgrade = 0;
	int32_t newlevel = 0;
	...
	if (m_level > 0){
		if (rnd <= 100 && rnd > 15){
			upgrade = 10;
			newlevel = m_level + upgrade;
		}
		else if (rnd <= 15 && rnd > 7){
			upgrade = 20;
			newlevel = m_level + upgrade;
		}
		else if (rnd <= 7 && rnd > 1){
	...
	if (newlevel > 0){
		std::string str = " -Lvl [" + std::to_string(newlevel) + "]";
		m_name = mType->name + str;
	}
	else{
		std::string str = " -[BOSS]";
		m_name = mType->name + str;		
	...
	m_nameDescription = mType->nameDescription;
	spawn = nullptr;
	...
	if (upgrade == 0){
		if (newlevel > 0){
			...
			skull = getSkullType("black");
			m_skull = skull;
			m_loot = 1;
		}
	}
	else if (upgrade == 10){
		skull = getSkullType("green");
		m_skull = skull;
		m_loot = 2;
	}
	else if (upgrade == 20){
		skull = getSkullType("yellow");
		m_skull = skull;
		m_loot = 3;
	}
	else if (upgrade == 30){
		skull = getSkullType("orange");
		m_skull = skull;
		m_loot = 5;
	}
	else if (upgrade == 40){
		skull = getSkullType("red");
		m_skull = skull;
		m_loot = 10;
	}

	m_experience = mType->experience * m_loot;
	health = mType->health * ((upgrade / 10) + 1);
	healthMax = mType->healthMax  * ((upgrade / 10) + 1);
	baseSpeed = mType->baseSpeed;
	m_plusPowerPercent = (upgrade / 10) + 1;
...
{
	if (corpse && lootDrop) {
		mType->mp_loot = getMonsterPlusLoot();
