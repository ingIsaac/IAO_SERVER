...
CombatDamage Combat::getCombatDamage(Creature* creature, Creature* target) const
{
	...
	if (creature->getCombatValues(min, max)) {
		if (Monster* monster = creature->getMonster()){
			...
			damage.primary.value = damage.primary.value * monster->getMonsterPlusPower();
		}
		else{
			damage.primary.value = normal_random(min, max);
		}
	...
	if (Player* player = creature->getPlayer()) {
		std::int32_t critical_value;
		std::int32_t plus_value;
		std::int32_t npdmg_value;
		player->getStorageValue(114968, critical_value);
		player->getStorageValue(112294, plus_value);
		player->getStorageValue(112354, npdmg_value);
		int32_t r = uniform_random(0, 100);	
		if (r <= critical_value){
			damage.primary.value = (damage.primary.value * 2) + plus_value;
			g_game.internalCreatureSay(player, TALKTYPE_MONSTER_SAY, "Critical!", false);
			player->sendTextMessage(MESSAGE_STATUS_DEFAULT, "[Critical!]");
		}
		newPercentDamage = damage.primary.value * (npdmg_value / 100);
		damage.primary.value = damage.primary.value + newPercentDamage;
	}
	return damage;
}
...
bool Combat::isProtected(const Player* attacker, const Player* target)
{
	...
	std::int32_t tar2;
	target->getStorageValue(21206, tar2);

	std::int32_t at2;
	attacker->getStorageValue(21206, at2);

	std::int32_t tar3;
	target->getStorageValue(21202, tar3);

	std::int32_t at3;
	attacker->getStorageValue(21202, at3);

	if ((tar2 == 1 && at2 == 0) || (tar2 == 0 && at2 == 1)){
		return true;
	}
	...
	if (tar2 == 1 && at2 == 1){
		...
		std::int32_t tar;
		target->getStorageValue(21201, tar);

		std::int32_t at;
		attacker->getStorageValue(21201, at);
		...
		std::int32_t tar4;
		target->getStorageValue(113966, tar4);

		std::int32_t at4;
		attacker->getStorageValue(113966, at4);
		if (tar4 > 0 && at4 > 0){
			if (tar4 == at4){
				return true;
			}
...
bool Combat::CombatHealthFunc(Creature* caster, Creature* target, const CombatParams& params, void* data)
{
	...
	if ((damage.primary.value < 0 || damage.secondary.value < 0) && caster) {
		...
		if (targetPlayer && caster->getPlayer() && targetPlayer->getSkull() != SKULL_BLACK) {
			...
			std::int32_t resilience_value;
			std::int32_t resilience_talent;
			targetPlayer->getStorageValue(112290, resilience_value);
			targetPlayer->getStorageValue(1712202, resilience_talent);
			if (resilience_value > 0){
				float v, x;
				v = (resilience_value / 1000);
				x = (resilience_talent / 100);
				damage.primary.value = damage.primary.value - (damage.primary.value*(v + x));
				damage.secondary.value = damage.secondary.value - (damage.secondary.value*(v + x));
			}
		}

		if (targetPlayer && caster->getCreature()) {			
			std::int32_t reduce_dmg;			
			targetPlayer->getStorageValue(1712301, reduce_dmg);
			if (reduce_dmg > 0){
				float v;
				v = (reduce_dmg / 100);
				damage.primary.value = damage.primary.value - (damage.primary.value*v);
				damage.secondary.value = damage.secondary.value - (damage.secondary.value*v);
			}
		}

		if (targetPlayer){
			std::int32_t dodge_value;
			std::int32_t block_value;
			targetPlayer->getStorageValue(112291, dodge_value);
			targetPlayer->getStorageValue(1712300, block_value);
			int32_t r = uniform_random(1, 100);
			int32_t b = uniform_random(1, 100);
			if (r <= dodge_value){
				g_game.internalCreatureSay(targetPlayer, TALKTYPE_MONSTER_SAY, "Dodge!", false);
				g_game.addMagicEffect(targetPlayer->getPosition(), CONST_ME_GROUNDSHAKER);
				return false;
			}
			else if (b <= block_value){
				g_game.internalCreatureSay(targetPlayer, TALKTYPE_MONSTER_SAY, "Block!", false);
				g_game.addMagicEffect(targetPlayer->getPosition(), CONST_ME_SHIELD_DEBUFF);
				return false;
			}
	    }
	}
...
bool Combat::CombatManaFunc(Creature* caster, Creature* target, const CombatParams& params, void* data)
{
	...
	if (damage.primary.value < 0) {
		...
		if (caster && caster->getPlayer() && target->getPlayer()) {
			damage.primary.value /= 2;
			Player* playerResilence = target->getPlayer();
			std::int32_t resilience_value;
			std::int32_t resilience_talent;
			playerResilence->getStorageValue(112290, resilience_value);
			targetPlayer->getStorageValue(1712202, resilience_talent);
			if (resilience_value > 0){
				float v, x;
				v = (resilience_value / 1000);
		...
		if (caster && caster->getCreature() && target->getPlayer()) {
			std::int32_t reduce_dmg;
			targetPlayer->getStorageValue(1712301, reduce_dmg);
			if (reduce_dmg > 0){
				float v;
